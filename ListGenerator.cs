...
namespace Demo1_LinQ_Assignment
{
    internal static class ListGenerator
    {
        ...
        static ListGenerator()
        {
            ...
            //CustomerList = XDocument.Load("C:\\Users\\ATTYA\\source\\repos\\Demo1-LinQ-Assignment\\Customers.Xml"); //   ممكن يتغير ف اي لحظه  Path    مش صح نعمل كدا لان ال 

            CustomerList = (from e in XDocument.Load("Customers.xml").Root.Elements("customer")
                             select new Customer()
                             {
                                 CustomerID = (string)e.Element("id"),          // Casting    لازم نعمل هنا 
                             ...
                             }).ToList();    // List  لازم نعمل هنا علشان يتشاف علي انها 
